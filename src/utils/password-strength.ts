...
// has number
const hasNumber: StringBoolFunc = (number:string) => new RegExp(/[0-9]/).test(number);

// has mix of small and capitals
const hasMixed: StringBoolFunc = (number:string) => new RegExp(/[a-z]/).test(number) && new RegExp(/[A-Z]/).test(number);

// has special chars
const hasSpecial: StringBoolFunc = (number:string) => new RegExp(/[!#@$%^&*)(+=._-]/).test(number);
